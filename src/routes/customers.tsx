    ...
    <div className="p-6 md:p-8 max-w-7xl mx-auto">
      <header className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            {customers.length} عميل في برنامج الولاء
          </p>
        </div>
        <button
          onClick={() => setAdding(true)}
          className="bg-primary text-white px-4 py-2.5 rounded-xl flex items-center gap-2 shadow-elegant hover:opacity-90 transition"
        >
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {customers.map((c) => (
          <div
            key={c.id}
            className="bg-card border border-border rounded-md p-5 shadow-soft hover:shadow-elegant transition"
          >
            ...
            <div className="grid grid-cols-3 gap-2 text-center">
              ...
              <div className="bg-success/10 rounded-xl py-2">
                <div className="text-xs font-bold text-success mt-1">
                  {c.totalSpent.toLocaleString('ar-EG')}
                </div>
      ...
      {adding && (
        ...
        >
          <div
            ...
            className="bg-card rounded-3xl p-6 w-full max-w-md shadow-elegant"
          >
            ...
            <div className="space-y-3">
              <input
                ...
                className="w-full bg-background border border-border rounded-xl py-3 px-4 focus:outline-none focus:ring-2 focus:ring-primary"
              />
              <input
                value={phone}
              ...
              <button
                onClick={submit}
                className="w-full gradient-primary text-primary-foreground py-3 rounded-xl font-medium hover:opacity-90"
              >
                إضافة
              </button>
            </div>
